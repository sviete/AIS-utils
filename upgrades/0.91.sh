...
echo "-----------------------------";
curl -o "/sdcard/ais-dom-0.91.4.tar.gz" -L https://raw.githubusercontent.com/sviete/AIS-utils/master/patches/scripts/ais-dom-0.91.4.tar.gz &&

curl --header "Content-Type: application/json" --max-time 2 --request POST --data '{"text":"Instaluje"}' http://localhost:8122/text_to_speech &&

pip install /sdcard/ais-dom-0.91.4.tar.gz -U &&
