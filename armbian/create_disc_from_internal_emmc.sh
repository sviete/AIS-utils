...
set_rootfs_type() {
    echo -e "${STEPS} Using ext4 type of filesystem for your root."
#cat <<EOF
#-----------------------------------------------
#  ID   TYPE
#-----------------------------------------------
...
#-----------------------------------------------
#EOF
    echo -ne "${OPTIONS} Please wait a moment: "
    filetype=1
...
echo -e "${STEPS} Creating disc on internal eMMC..."
...
echo "Add a new entry for the mount point"
sed -i '\?^/dev/mmcblk2p2?d' /etc/fstab
echo "/dev/mmcblk2p2 	/media/d2 ext4  defaults        0       0" >> /etc/fstab

echo "Mount all filesystems listed in the /etc/fstab file"
mount -a
df -h

echo -e "${SUCCESS} Creation new disk successful."
exit 0
